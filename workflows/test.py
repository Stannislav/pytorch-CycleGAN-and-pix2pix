...
def main():
    ...
    # test code only supports num_threads = 0
    opt.num_threads = 0
    ...
    # test with eval mode. This only affects layers like batch-norm and dropout.
    # For [pix2pix]: we use batch-norm and dropout in the original pix2pix.
    # You can experiment it with and without eval() mode.
    # For [CycleGAN]: It should not affect CycleGAN as CycleGAN uses instance-norm without dropout.
    if opt.eval:
        model.eval()
